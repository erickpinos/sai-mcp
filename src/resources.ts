...
- **No minimum position size.** There is NO enforced minimum notional. A market's \`minPositionSizeUSD\` is ADVISORY ONLY and NOT enforced on-chain — the indexer reports the same flat value for every market and the chain accepts positions far below it (sub-$1, ~$0.02 notional observed). \`minLeverage\` is 1 on the major markets, so open a small position at LOW leverage; never inflate the user's requested leverage to "reach the minimum." The authoritative size check is the dry-run gas estimate (a too-small position sets \`gas.estimationError\`).
